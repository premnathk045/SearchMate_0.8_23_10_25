...
let masterClosedTabs = []; // Array to store tabs that have been closed but not yet processed by AI
let unprocessedTabs = []; // Tabs waiting for AI grouping
...
const USER_CREDITS_KEY = 'userCredits'; // NEW: Constant for Credit Storage (using chrome.storage.sync so the credits are synced across all devices)

let userCredits = 100; // Default/initial value, will be overwritten by storage.

// --- Credit Utility Functions ---

/*** Calculates the credits to deduct based on token count (Base Rate: 1 Credit per 1000 tokens, rounded up).
 * @param {number} tokenCount The totalTokenCount from the Gemini API response.
 * @returns {number} The calculated credits to deduct.
 */
function calculateCredits(tokenCount) {
    if (tokenCount <= 0 || !tokenCount) return 0;
    // Credits Used = ceil(tokenCount / 1000)
    return Math.ceil(tokenCount / 1000);
}

/*** Loads the current user credit balance from chrome.storage.sync.
 * @returns {Promise<number>} The current credit balance.
 */
async function loadUserCredits() {
    try {
        const result = await chrome.storage.sync.get([USER_CREDITS_KEY]);
        // Default to 100 if not found
        userCredits = result[USER_CREDITS_KEY] !== undefined ? result[USER_CREDITS_KEY] : 100;
        return userCredits;
    } catch (error) {
...
/*** Saves the new user credit balance to chrome.storage.sync.
 * @param {number} newBalance The updated credit balance.
 * @returns {Promise<void>}
 */
async function saveUserCredits(newBalance) {
    userCredits = Math.max(0, newBalance); // Ensure credits don't drop below zero
    try {
        await chrome.storage.sync.set({ [USER_CREDITS_KEY]: userCredits });
        console.log(`User credits saved: ${userCredits}`);
        // Send a message to the sidepanel to update the UI
        chrome.runtime.sendMessage({ action: "updateCredits", newBalance: userCredits });
    } catch (error) {
 ...
 * @returns {Promise<object>} The JSON response from the API (includes usageMetadata).
 */
async function handleFetchWithRetry(url, options) {
    ...
    if (isCustomEnabled && storageResult[CUSTOM_PROMPT_KEY]) {
        systemPromptOverride = `\n\n---USER CUSTOM INSTRUCTION---${storageResult[CUSTOM_PROMPT_KEY]}------------------------------`;
    }
    ...
    for (let i = 0; i < MAX_RETRIES; i++) {
        try {
            ...
            return await response.json(); // Return the full response object, including usageMetadata
 ...
 * @returns {Promise<object>} Grouped tabs structure compatible with sidepanel.js, or an error object with fallback data.
 */
async function groupTabsWithAI_FullRegroup(tabs) {
    // 1. Pre-Check: Credit Check (Minimum 1 credit)
    const initialCredits = await loadUserCredits();
    if (initialCredits < 1) {
        console.error("AI grouping skipped: Insufficient credits.");
        return { groups: [], ungrouped: tabs, error: "Insufficient credits for full grouping. Minimum 1 required." };
    }
    ...
    const tabList = tabs.map(tab => ({ id: tab.id.toString(), title: tab.title, url: stripUrlNoise(tab.url) }));
...
${JSON.stringify(tabList, null, 2)}`;
    ...
    // 3. Define the expected JSON response schema
    const responseSchema = {
        type: "OBJECT", properties: {
            "groups": {
                        ...
                        "subgroups": {
                            "type": "ARRAY", "items": {
                                "type": "OBJECT", "properties": {
                                    ...
                                    "tabs": { "type": "ARRAY", "items": { "type": "OBJECT", "properties": { "id": { "type": "STRING", "description": "The unique string ID of the tab from the input list." } }, "propertyOrdering": ["id"] } }
                ...
                "type": "ARRAY", "items": {
                    "type": "OBJECT", "properties": { "id": { "type": "STRING", "description": "The unique string ID of a tab that couldn't be logically grouped." } }, "propertyOrdering": ["id"]
                }
            }
    ...
    try {
        ...
        // 3. Token Retrieval & 4. Credit Calculation
        const totalTokenCount = apiResponse.usageMetadata?.totalTokenCount || 0;
        const creditsToDeduct = calculateCredits(totalTokenCount);
        console.log(`Full Regroup consumed ${totalTokenCount} tokens, resulting in ${creditsToDeduct} credits deducted.`);

        // 5. Deduction and Storage
        await saveUserCredits(initialCredits - creditsToDeduct);
        ...
        // 4. Rehydrate the grouped ID with full tab data
        ...
        const finalGroups = aiGroupedData.groups.map(group => ({
            ...group, subgroups: group.subgroups.map(subgroup => ({
                ...subgroup, tabs: subgroup.tabs.map(tabIdObj => tabMap.get(tabIdObj.id)).filter(tab => tab) // Filter out any IDs the model hallucinated
            }))
        }));
        ...
        // Fallback: if AI fails, return all tabs as ungrouped. No deduction happens on fetch failure.
        return { groups: [], ungrouped: tabs, error: error.message };
    }
}

/*** NEW FUNCTION: Handles the AI-Assisted Contextual Integration (Incremental Grouping).
 * @returns {Promise<object>} The final merged grouped tabs structure, or an error object with fallback data.
 */
async function incrementalGroupTabs() {
    ...
    if (initialCredits < 1) {
        console.error("AI grouping skipped: Insufficient credits.");
        const storage = await chrome.storage.local.get([GROUPED_TABS_KEY]);
        // Return existing data plus error flag/message
        return { ...storage[GROUPED_TABS_KEY] || { groups: [], ungrouped: [] }, error: "Insufficient credits for incremental grouping. Minimum 1 required." };
    }
    ...
    const compressedStructure = existingGroupedTabs.groups.map(group => ({
        title: group.title,
        subgroups: group.subgroups.map(subgroup => ({ title: subgroup.title, existingTabIds: subgroup.tabs.map(tab => tab.id) }))
    }));

    const newTabsContext = newTabs.map(tab => ({ id: tab.id.toString(), title: tab.title, url: stripUrlNoise(tab.url) }));

    // --- Step 2: AI Prompt and Action Generation ---
    const userQuery = `Analyze the following NEW TABS and the EXISTING TAB GROUPS structure. Your task is to generate a list of precise, sequential actions to integrate every single New Tab into the existing structure, create new groups/subgroups, or mark it as ungrouped. IMPORTANT: You MUST generate exactly one action object for every New Tab ID provided. Do NOT change any existing data or include existing tab IDs in your output.
...
${JSON.stringify(newTabsContext, null, 2)}`;
    ...
    const systemPrompt = `You are an expert AI Tab Manager focused on incremental data merging. Return only a clean JSON object containing an 'actions' array. Each action in the array MUST be an object with a 'tabId'(string from NEW TABS) and a 'type'. Acceptable Action Types: 1. APPEND_SUBGROUP: Requires 'targetGroupTitle'(string) and 'targetSubgroupTitle'(string). Use only for existing subgroups. 2. INSERT_SUBGROUP: Requires 'targetGroupTitle'(string) and 'newSubgroupTitle'(string). Use to create a new subgroup within an existing group. 3. INSERT_GROUP: Requires 'newGroupTitle'(string) and 'newSubgroupTitle'(string). Use to create a brand new main group. 4. UNGROUPED: Places the tab in the top-level ungrouped list. No extra properties needed. Titles must be concise. Prioritize APPEND_SUBGROUP if contextually relevant.`;
    ...
    try {
        ...
        const totalTokenCount = apiResponse.usageMetadata?.totalTokenCount || 0;
        const creditsToDeduct = calculateCredits(totalTokenCount);
        console.log(`Incremental Integration consumed ${totalTokenCount} tokens, resulting in ${creditsToDeduct} credits deducted.`);
        ...
        return { ...existingGroupedTabs, error: error.message };
    }
    ...
    updatedGroupedTabs.groups.forEach(group => { group.subgroups.forEach(subgroup => { subgroup.tabs.forEach(tab => existingIds.add(tab.id.toString())); }); });
    ...
    for (const action of aiActions) {
        ...
        const newTab = { id: tabData.id.toString(), title: tabData.title, url: tabData.url, favicon: tabData.favicon, closedTimestamp: tabData.closedTimestamp };
        try {
            switch (action.type) {
                case "APPEND_SUBGROUP": {
                    ...
                    if (group) {
                        ...
                        } else { // Fallback: create new subgroup if AI suggested append but subgroup doesn't exist
                            group.subgroups.push({ title: action.targetSubgroupTitle, tabs: [newTab] });
                        }
                    } else { // Fallback: create new group if AI suggested append but group doesn't exist
                ...
                case "INSERT_SUBGROUP": {
                    ...
                    } else { // Fallback: create new group/subgroup if target group doesn't exist
                        updatedGroupedTabs.groups.push({
                            title: action.targetGroupTitle, // Use target title as new group title
                ...
                case "INSERT_GROUP": {
                    updatedGroupedTabs.groups.push({ title: action.newGroupTitle, subgroups: [{ title: action.newSubgroupTitle, tabs: [newTab] }] });
                    break;
                }
                case "UNGROUPED": {
    ...
    // Finalize storage: Save the updated grouped tab and clear the processed list
...
function loadStoredTabs() {
    // Load local storage items and sync storage items concurrently
    chrome.storage.local.get([MASTER_TABS_KEY, UNPROCESSED_TABS_KEY], (localResult) => {
        chrome.storage.sync.get([USER_CREDITS_KEY], (syncResult) => {
            try {
                // Load master closed tabs list
                const storedTabs = localResult[MASTER_TABS_KEY] || [];
                masterClosedTabs = storedTabs.slice(0, 100);

                // Load tabs waiting for processing
                unprocessedTabs = localResult[UNPROCESSED_TABS_KEY] || [];

                // Load and set user credits (sync storage)
                userCredits = syncResult[USER_CREDITS_KEY] !== undefined ? syncResult[USER_CREDITS_KEY] : 100;
                console.log(`Initial User credits loaded: ${userCredits}`);

                // If credits were just initialized (i.e., not found in storage), save the initial value
                if (syncResult[USER_CREDITS_KEY] === undefined) {
                    saveUserCredits(100);
                }

            } catch (error) {
                console.error('Error loading stored tabs/credits:', error);
                masterClosedTabs = [];
                unprocessedTabs = [];
                userCredits = 100; // Reset local state on fatal error
            }
        });
    });
}
...
async function restoreGroupingFromBackup() {
    try {
        ...
        if (result.backup_groupedTabs?.data) {
            ...
            if (JSON.stringify(verify.searchmate_groupedTabs) !== JSON.stringify(result.backup_groupedTabs.data)) {
                throw new Error('Restore verification failed');
            }
            return { status: "success", message: "Previous tab grouping restored successfully", timestamp: result.backup_groupedTabs.timestamp };
        }
        return { status: "error", message: "No backup found" };
    } catch (error) {
        console.error('Error restoring backup:', error);
        return { status: "error", message: "Failed to restore backup:" + error.message };
    }
}
    ...
    if (info.menuItemId === "add-to-notes") {
        ...
        chrome.storage.local.get(['searchmate_notes'], (result) => {
            ...
            chrome.storage.local.set({ searchmate_notes: notes }, () => {
                // Notify the side panel to refresh notes
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    switch (request.action) {
        ...
        case "getClosedTabs":
            ...
            (async () => {
                // Load the current credit balance BEFORE starting grouping
                const initialCredits = await loadUserCredits();
                ...
                let groupedTabs;
                let error = null; // Variable to capture potential credit/API error

                if (!useIncrementalGrouping) {
                    // Mode 1: Full Regroup (Existing Architecture)
                    if (request.forceRegroup || !result[GROUPED_TABS_KEY]) {
                        console.log('backuptrigger02');
                        await backupCurrentGrouping();
                        await chrome.storage.local.remove(UNPROCESSED_TABS_KEY);

                        // groupTabsWithAI_FullRegroup handles its own credit check and deduction
                        groupedTabs = await groupTabsWithAI_FullRegroup(masterTabs);
                        if (groupedTabs.error) {
                            error = groupedTabs.error;
                            // Use fallback structure for display in case of error
                            groupedTabs = { groups: [], ungrouped: masterTabs };
                        } else {
                    ...
                    // incrementalGroupTabs handles its own credit check and deduction
                    groupedTabs = await incrementalGroupTabs();
                    if (groupedTabs.error) {
                        error = groupedTabs.error;
                        // groupedTabs already contains the existing structure plus the error message
                        groupedTabs = { groups: groupedTabs.groups, ungrouped: groupedTabs.ungrouped };
                    }
                ...
                sendResponse({
                    ...
                    userCredits: await loadUserCredits(), // Send the final/updated credit balance
                    error: error // Send error message if applicable
                });
            })();
        ...
        case "updateCredits":
            // Case to handle initial credit load/update request from sidepanel on load
            (async () => {
                sendResponse({ newBalance: await loadUserCredits() });
            })();
            return true;
...
function updateActiveTabInSidepanel(tab) {
    chrome.runtime.sendMessage({ action: "updateActiveTab", tabId: tab.id.toString(), url: tab.url, title: tab.title // Add the title for more accurate matching
    });
}
...
async function openTabsInGroup(tabs, groupTitle) {
    try {
        ...
        activeTabGroups.set(groupId, {
            title: groupTitle,
            tabs: createdTabs.map(tab => ({ id: tab.id.toString(), title: tab.title || "New Tab", url: tab.url, favicon: tab.favIconUrl }))
        });
...
chrome.tabs.onCreated.addListener(async (tab) => {
    try {
        ...
        if (!activeTabGroups.has(groupId)) {
            activeTabGroups.set(groupId, { title: group.title, tabs: [] });
        }
...
function stripUrlNoise(url) {
    try {
        ...
        console.log('Stripped URL:', u.toString());
        return u.toString();
    } catch {
        return url;
    }
